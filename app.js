...
const TK = (function () {
  ...
  /* ------------------------------------------------------------------
     ...
     A person can keep several lists — a weekly shop, a dinner party,
     a holiday. One is "active" at a time.

     Shape: {
       v: 2,
       active: "id",
       order: ["id", ...],                    display order of the tabs
       lists: { id: { id, name, recipes:[{id,servings}],
                      manual:[string], checked:{key:true}, created } },
       aisles: ["Produce", ...]               custom shop-walking order
     }

     getGrocery() still returns just the active list, exactly as it did
     when there was only one, so nothing that reads a list had to change.
     ------------------------------------------------------------------ */
  ...
  function blankList(name) {
    return { id: newId(), name: name || "My list", recipes: [], manual: [],
             checked: {}, created: Date.now() };
  }

  /* Reads the whole store, upgrading the old single-list format on the way
     through so nobody loses a list they had already started. */
  function getGroceryStore() {
    let s = read(KEYS.grocery, null);

    if (!s || typeof s !== "object" || Array.isArray(s)) s = null;

    /* Version 1 had recipes/manual/checked at the top level. Only treat it as
       a real v1 list if it actually carries v1 data; anything else is junk
       and gets replaced with a clean store rather than half-adopted. */
    if (s && !s.lists && !Array.isArray(s.recipes) && !Array.isArray(s.manual)) s = null;
    if (s && !s.lists) {
      const carried = blankList("My list");
      carried.recipes = Array.isArray(s.recipes) ? s.recipes : [];
      carried.manual  = Array.isArray(s.manual) ? s.manual : [];
      carried.checked = (s.checked && typeof s.checked === "object") ? s.checked : {};
      s = { v: 2, active: carried.id, order: [carried.id],
            lists: {}, aisles: null };
      s.lists[carried.id] = carried;
      write(KEYS.grocery, s);
    }

    if (!s) {
      const first = blankList("My list");
      s = { v: 2, active: first.id, order: [first.id], lists: {}, aisles: null };
      s.lists[first.id] = first;
      write(KEYS.grocery, s);      // persist immediately, so the next read agrees
    }

    /* Self-healing: drop anything malformed rather than throwing. */
    if (!s.lists || typeof s.lists !== "object") s.lists = {};
    if (!Array.isArray(s.order)) s.order = [];
    s.order = s.order.filter(function (id) { return !!s.lists[id]; });
    Object.keys(s.lists).forEach(function (id) {
      if (s.order.indexOf(id) === -1) s.order.push(id);
      const l = s.lists[id];
      l.id = id;
      if (typeof l.name !== "string" || !l.name.trim()) l.name = "Untitled list";
      if (!Array.isArray(l.recipes)) l.recipes = [];
      if (!Array.isArray(l.manual)) l.manual = [];
      if (!l.checked || typeof l.checked !== "object") l.checked = {};
      l.recipes = l.recipes.filter(function (e) { return e && getRecipe(e.id); });
    });
    if (!s.order.length) {
      const first = blankList("My list");
      s.lists[first.id] = first;
    ...
    if (!s.lists[s.active]) s.active = s.order[0];
    return s;
  }

  function saveGroceryStore(s) { write(KEYS.grocery, s); }

  /* The active list, in the same shape the rest of the site already expects.
     Reads the store exactly once — calling getGroceryStore() twice can mint
     two different fresh stores on a first visit, and the lookup then misses. */
  function getGrocery() {
    const s = getGroceryStore();
    return s.lists[s.active];
  }

  function saveGrocery(list) {
    const s = getGroceryStore();
    const id = list.id && s.lists[list.id] ? list.id : s.active;
    list.id = id;
    s.lists[id] = list;
    saveGroceryStore(s);
  }
  ...
  function groceryLists() {
    ...
    return s.order.map(function (id) {
      const l = s.lists[id];
      return { id: id, name: l.name, active: id === s.active,
               recipes: l.recipes.length, manual: l.manual.length };
    });
  }

  function activeListId() { return getGroceryStore().active; }

  function setActiveList(id) {
    const s = getGroceryStore();
    if (!s.lists[id]) return false;
    s.active = id;
    saveGroceryStore(s);
    return true;
  }
  ...
  function renameList(id, name) {
    const s = getGroceryStore();
    if (!s.lists[id] || !name || !name.trim()) return false;
    s.lists[id].name = name.trim().slice(0, 60);
    saveGroceryStore(s);
    return true;
  }
  ...
  function duplicateList(id) {
    const s = getGroceryStore();
    const src = s.lists[id];
    if (!src) return null;
    const copy = blankList(src.name + " (copy)");
    copy.recipes = src.recipes.map(function (r) { return { id: r.id, servings: r.servings }; });
    copy.manual = src.manual.slice();
    /* A copy starts unticked — it's a fresh shop, not a record of the old one. */
    s.lists[copy.id] = copy;
  ...
  function deleteList(id) {
    const s = getGroceryStore();
    if (!s.lists[id]) return false;
    if (s.order.length === 1) {           // never leave them with none
      s.lists[id] = blankList(s.lists[id].name);
      s.lists[id].id = id;
      s.order = [id];
      s.active = id;
      saveGroceryStore(s);
    ...
    delete s.lists[id];
    s.order.splice(at, 1);
    if (s.active === id) s.active = s.order[Math.max(0, at - 1)];
    saveGroceryStore(s);
    return true;
  }

  /* --- aisle order --------------------------------------------------
     Supermarkets don't agree on layout, so the walking order is the
     shopper's to set. Falls back to the order in recipes.js. */
  function getAisleOrder() {
    const s = getGroceryStore();
    const saved = Array.isArray(s.aisles) ? s.aisles : [];
    const valid = saved.filter(function (g) { return GROCERY_GROUPS.indexOf(g) !== -1; });
    GROCERY_GROUPS.forEach(function (g) {
      if (valid.indexOf(g) === -1) valid.push(g);   // new aisles land at the end
    });
    return valid;
  }

  function setAisleOrder(order) {
    const s = getGroceryStore();
    s.aisles = order.filter(function (g) { return GROCERY_GROUPS.indexOf(g) !== -1; });
    saveGroceryStore(s);
  }

  function moveAisle(group, direction) {
  ...
  /* --- sharing a list to another device ------------------------------
     The list is packed into the part of the URL after the #, which
     browsers never send to a server. Nothing leaves the device except
     through the link the person chooses to send. */
  function encodeList(list) {
    const payload = {
      n: list.name,
      r: list.recipes.map(function (r) { return [r.id, r.servings]; }),
      m: list.manual.slice()
    };
  ...
  function decodeList(code) {
    try {
      ...
      return {
        name: typeof data.n === "string" ? data.n.slice(0, 60) : "Shared list",
        recipes: data.r
          .filter(function (p) { return Array.isArray(p) && getRecipe(p[0]); })
          .map(function (p) { return { id: p[0], servings: Number(p[1]) || 1 }; }),
        manual: Array.isArray(data.m)
          ? data.m.filter(function (x) { return typeof x === "string"; }).slice(0, 100)
          : []
      };
    } catch (e) { return null; }
  }
  ...
  function importList(data) {
    const s = getGroceryStore();
    const l = blankList(data.name);
    l.recipes = data.recipes;
    l.manual = data.manual;
    s.lists[l.id] = l;
    s.order.push(l.id);
    s.active = l.id;
    saveGroceryStore(s);
  ...
  function buildGroceryList() {
    ...
    // Group into aisles, in whatever walking order the shopper has set
    const grouped = [];
    getAisleOrder().forEach(function (groupName) {
  ...
  return {
    ...
    groceryLists: groceryLists, activeListId: activeListId, setActiveList: setActiveList,
    createList: createList, renameList: renameList, duplicateList: duplicateList,
    deleteList: deleteList,
    getAisleOrder: getAisleOrder, setAisleOrder: setAisleOrder, moveAisle: moveAisle,
    encodeList: encodeList, decodeList: decodeList, importList: importList,
